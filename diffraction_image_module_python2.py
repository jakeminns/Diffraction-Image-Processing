...
################# .img Related Functions ###########################
	...
	def imgHeaderRead(self):
		...
		for line in header:
			lineSplit = (filter(None,re.split('	|=| ',line))) #split acording to spaces and equal symbol then filter blank entries
...
############## .cbf Related Functions ########################################################
	...
	def cbfReaderCryio(self):
		self.typeCheck(".cbf")
		self.cryioImg = CbfImage(self.filePath)
		print(type(self.cryioImg.array))
		self.array = self.cryioImg.array
		self.xN = self.cryioImg.binary_header_dict['X-Binary-Size-Second-Dimension']
		self.xY = self.cryioImg.binary_header_dict['X-Binary-Size-Fastest-Dimension']

	def newCbFImageCryio(self,path):
		self.cryioImg.array = self.array
		self.cryioImg.save_cbf(filePath,"cbf")
	...
	def cbfReader(self):
		self.typeCheck(".cbf")
		image = self.cbfGetImage()
		return 0, image.width(), image.height(), image.data() # 0 at the beginning for headerSize
	...
	def newCbfImage(self,path):
		self.typeCheck(".cbf")
		#image.setOptionalData(optionalData)
		cbfImg = self.array2CbfImage(self.array)
		writer = dectris.albula.DImageWriter(path+".cbf", format="cbf")
		writer.write(cbfImg)
...
############# Image Manipulation ####################################
	...
	def saveImage(self,path):
		if self.imgType == ".cbf":
			self.newCbFImageCryio(path)
		if self.imgType == ".img":
			self.newImgImage(path)

	def divideImage(self,divider):
		np.divide(self.array,divider,out = self.array)
...
class ToolsD:
	...
	def backgroundSubtraction(self,lowCut,highCut,seriesD):
		...
		#cutD.medianFilterSeries(0,cutD.numImages,50)
